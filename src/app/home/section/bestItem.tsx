    ...
    <div className="w-full h-auto lg:h-screen flex flex-col lg:flex-row-reverse relative lg:justify-center pt-6 lg:pt-24 px-4 lg:px-0">
      ...
      <div className="w-full lg:w-2/5 text-center lg:text-left">
        <div className="text-black text-3xl sm:text-4xl font-bold">
          {page && page[0]?.[locale]?.[2]?.title || <Spinner label="Loading" color="primary" labelColor="primary" />}
        </div>
        <div className="text-black text-sm text-justify mt-2 lg:mt-10">
          {page && page[0]?.[locale]?.[2]?.content || <Spinner label="Loading" color="primary" labelColor="primary" />}
        </div>
        {/* stats sections when */}
        <div className="hidden lg:flex bg-white text-black lg:w-fit shadow-2xl justify-between rounded-lg py-3 px-12 mt-6">
          <div className="border-r-2 pr-2 lg:pr-6 my-4 lg:my-6 border-black flex flex-col items-center">
            <div className="text-sm lg:text-3xl font-semibold">{page && page[0]?.[locale]?.[2]?.bestNumber1 || <Spinner label="Loading" color="primary" labelColor="primary" />}</div>
          ...
          <div className="px-2 lg:px-6 my-4 lg:my-6 flex flex-col items-center">
            <div className="text-sm lg:text-3xl font-semibold">{page && page[0]?.[locale]?.[2]?.bestNumber2 || <Spinner label="Loading" color="primary" labelColor="primary" />}</div>
          ...
          <div className="border-l-2 pl-2 lg:pl-6 my-4 lg:my-6 border-black flex flex-col items-center">
            <div className="text-sm lg:text-3xl font-semibold">{page && page[0]?.[locale]?.[2]?.bestNumber3 || <Spinner label="Loading" color="primary" labelColor="primary" />}</div>
